...
function getDefaultScaleFactor() {
	// 为录制/截图提供更清晰的默认渲染：保持 1 倍缩放（用户仍可在设置里调节）
	return 1;
}
...
//随机文字烟花内容
const randomWords = ["HAPPY NEW YEAR", 
	"新年快乐", 
	"小秦同学，新年快乐！",
	"小秦同学，新年快乐！",
	"小秦同学，新年快乐！",
...
// 控制文字烟花出现概率
const WORD_SHELL_PROB = 0.25; // 25% 概率触发文字烟花
...
const appNodes = {
	...
	recordBtnText: ".record-btn__text",
...
function handleStateChange(state, prevState) {
	...
	if (canPlaySound !== canPlaySoundPrev) {
		...
		if (canPlaySound) {
			console.log('🔊 启用声音，音频上下文状态:', soundManager.ctx.state);
			
			// 立即resume AudioContext，利用用户点击事件
			if (soundManager.ctx.state === 'suspended') {
				soundManager.ctx.resume().then(() => {
					console.log('✅ 音频上下文已启动:', soundManager.ctx.state);
			...
			// 仅当 WebAudio 背景音乐不可用时，才使用 HTMLAudio 作为回退，避免双重播放。
			if (!soundManager.bgMusicBuffer && bgMusic) {
				bgMusic.volume = 0.3;
				bgMusic.play().catch(e => {
					console.log('🎵 HTML音乐播放需要用户交互:', e.message);
				});
			}
		} else {
			console.log('🔇 禁用声音');
...
function handlePointerStart(event) {
	...
	if (event.y < btnSize) {
		const stageRect = mainStage.canvas.getBoundingClientRect();
		const clientX = stageRect.left + event.x;
		const clientY = stageRect.top + event.y;

		const isInRect = (rect) => clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom;
		const hit = (node) => node && isInRect(node.getBoundingClientRect());

		if (hit(appNodes.pauseBtn)) {
			togglePause();
			return;
		}
		if (hit(appNodes.soundBtn)) {
			toggleSound();
			return;
		}
		if (hit(appNodes.recordBtn)) {
			if (window.recordingManager) {
				window.recordingManager.toggle();
			}
			return;
		}
		if (hit(appNodes.settingsBtn)) {
			toggleMenu();
			return;
		}
	}
...
class Shell {
	...
	burst(x, y) {
		...
		if (!this.disableWord && store.state.config.wordShell) {
			if (Math.random() < WORD_SHELL_PROB) {
		...
		if (this.comet) {
			...
			if (this.shellSize >= 2) {
				...
				setTimeout(() => {
					if (canPlaySoundSelector()) {
						soundManager.playSound("crackle", soundScale * 0.8);
					}
				}, 50 + Math.random() * 50);
				...
				setTimeout(() => {
					if (canPlaySoundSelector()) {
						soundManager.playSound("crackleSmall", soundScale * 0.6);
					}
				}, 120 + Math.random() * 80);
			}
			
			// 特定烟花类型的额外音效
			if (this.crackle || this.crossette) {
				// 爆裂型烟花：添加连续的爆裂声
				for (let i = 0; i < 3; i++) {
					setTimeout(() => {
						if (canPlaySoundSelector()) {
							soundManager.playSound("crackleSmall", soundScale * (0.5 + Math.random() * 0.3));
						}
					}, 80 * i + Math.random() * 60);
				}
			}
			...
			// 大型复杂烟花：添加小爆炸音效增强层次感
			if (this.pistil || this.ring) {
				setTimeout(() => {
					if (canPlaySoundSelector()) {
						soundManager.playSound("burstSmall", soundScale * 0.7);
					}
				}, 60 + Math.random() * 40);
			}
		}
...
const soundManager = {
	...
	bgMusicFile: "Happy-New-Year-A-Teens.mp3",
	bgMusicVolume: 0.3,
	sources: {
		lift: {
			volume: 1,
	...
	preload() {
		...
		console.log(`🎵 音频加载模式: ${isFileProtocol ? 'file:// (本地文件)' : 'http:// (服务器)'}`);

		// 加载单个音频文件的通用函数
		const loadAudioFile = (fileURL) => {
			if (isFileProtocol) {
				// file:// 协议：使用 XMLHttpRequest (兼容性更好)
				return new Promise((resolve, reject) => {
					...
					xhr.onload = () => {
						if (xhr.status === 200 || xhr.status === 0) { // file:// 协议返回 0
							this.ctx.decodeAudioData(
								xhr.response,
								(buffer) => resolve(buffer),
								(err) => reject(new Error(`音频解码失败: ${fileURL}`))
							);
						} else {
					...
					xhr.onerror = () => reject(new Error(`网络错误: ${fileURL}`));
					xhr.send();
				});
			} else {
				// http:// 协议：使用 fetch (标准方式)
		...
		// 预加载背景音乐
		const bgMusicPromise = loadAudioFile(this.baseURL + this.bgMusicFile)
			.then(buffer => {
				this.bgMusicBuffer = buffer;
				console.log('✅ 背景音乐加载成功');
			})
			.catch(err => {
				console.warn('❌ 背景音乐加载失败:', err.message);
			});
		allFilePromises.push(bgMusicPromise);
		...
		types.forEach((type) => {
			...
			const filePromises = [];
			// 记录原始URL，用于fallback
			source.urls = fileNames.map((fileName) => this.baseURL + fileName);
			
			fileNames.forEach((fileName) => {
				const fileURL = this.baseURL + fileName;
				const promise = loadAudioFile(fileURL)
					.catch(err => {
						console.warn(`❌ 音效加载失败 [${type}/${fileName}]:`, err.message);
						return null; // 返回null而不是失败，允许部分音效加载失败
					});
			...
			Promise.all(filePromises).then((buffers) => {
				...
				if (source.buffers.length > 0) {
					console.log(`✅ 音效加载成功 [${type}]: ${source.buffers.length}/${fileNames.length} 个文件`);
				}
			});
		});
	...
	resumeAll() {
		console.log('🔊 resumeAll 被调用 (已废弃，由handleStateChange处理)');
		// 注意：这个方法现在不再使用，因为Chrome要求AudioContext必须在用户交互时立即resume
		// 所有逻辑已移至handleStateChange中处理
	},
	...
	playSound(type, scale = 1) {
		...
		if (!canPlaySoundSelector() || simSpeed < 0.95) {
			if (scale > 0) { // 只在非静音时记录
				console.log(`🔇 音效被阻止: ${type}, canPlaySound=${canPlaySoundSelector()}, simSpeed=${simSpeed}`);
			}
			return;
		}
		...
		// 缓冲/URL可用性检查
		const hasBuffers = source.buffers && source.buffers.length > 0;
		const hasUrls = source.urls && source.urls.length > 0;
		if (!hasBuffers && !hasUrls) {
			console.warn(`⚠️  音效 "${type}" 的音频文件未加载或加载失败`);
			return;
		}
		...
		if (hasBuffers) {
			...
			// HTMLAudio回退，兼容file://
			const url = MyMath.randomChoice(source.urls);
			const audio = new Audio(url);
			audio.volume = scaledVolume;
			audio.playbackRate = scaledPlaybackRate;
			audio.play().catch((err) => console.warn(`⚠️  HTMLAudio 播放失败: ${url}`, err));
			playMethod = 'HTMLAudio';
		}
		
		if (scale > 0) { // 只在非静音时记录
			console.log(`🔊 播放音效: ${type}, 音量: ${scaledVolume.toFixed(2)}, 上下文状态: ${this.ctx.state}, 方式: ${playMethod}`);
		}
	},
	...
	playBgMusic() {
		...
		if (this.bgMusicBuffer) {
			...
			this.bgMusicGainNode.gain.value = this.bgMusicVolume;

			this.bgMusicSource.connect(this.bgMusicGainNode);
			this.bgMusicGainNode.connect(this.getOutputNode());

			this.bgMusicSource.start(0);
			console.log('🎵 背景音乐播放 (AudioBuffer)');
		} else {
			// HTMLAudio回退
			const bgMusicEl = document.getElementById('bgMusic');
			if (bgMusicEl) {
				bgMusicEl.volume = this.bgMusicVolume;
				bgMusicEl.loop = true;
				bgMusicEl.play().then(() => {
					console.log('🎵 背景音乐播放 (HTMLAudio)');
				}).catch((err) => {
					console.warn('⚠️  背景音乐播放失败 (HTMLAudio):', err.message);
				});
			}
		}
	},
...
// Expose for console scripts / debugging.
window.soundManager = soundManager;

// 录制管理器：合成两层 canvas 并输出高码率 WebM（可录制 WebAudio 音频）。
const recordingManager = (() => {
	const FPS = 60;
	// 1512x982（DIP）在 Retina 上常见会接近 ~3024x1964 像素源；烟花细节多，建议提高码率。
	const VIDEO_BPS = 40_000_000; // 40 Mbps
	const AUDIO_BPS = 192_000;
	const FILE_NAME = 'fireworks-recording.webm';
	...
	function setButtonUI(recording) {
		if (!appNodes.recordBtnText || !appNodes.recordBtn) return;
		appNodes.recordBtnText.textContent = recording ? 'STOP' : 'REC';
		appNodes.recordBtn.classList.toggle('is-recording', recording);
	}
	...
	function ensureAudioDestination() {
		...
		if (!audioDest) {
			...
			try {
				soundManager.getOutputNode().connect(audioDest);
			} catch (e) {
				console.warn('⚠️ 无法连接音频到录制目的地：', e);
			}
		}
		return audioDest;
	}
	...
	function start() {
		...
		const trails = trailsStage?.canvas;
		const main = mainStage?.canvas;
		const container = appNodes.canvasContainer;
		if (!trails || !main || !container) {
			console.warn('未找到必要元素：trails/main/canvasContainer');
			return;
		}
		...
		const w = Math.max(trails.width, main.width);
		const h = Math.max(trails.height, main.height);
		composite = document.createElement('canvas');
		composite.width = w;
		composite.height = h;
		const ctx2d = composite.getContext('2d', { alpha: false });
		if (!ctx2d) {
			console.warn('无法创建 2D context');
			composite = null;
			return;
		}

		const draw = () => {
			const bg = getComputedStyle(container).backgroundColor || 'rgb(0,0,0)';
			ctx2d.globalCompositeOperation = 'source-over';
			ctx2d.fillStyle = bg;
			ctx2d.fillRect(0, 0, w, h);

			ctx2d.globalCompositeOperation = 'lighten';
			ctx2d.drawImage(trails, 0, 0, w, h);
			ctx2d.drawImage(main, 0, 0, w, h);

			rafId = requestAnimationFrame(draw);
		};
		draw();

		videoStream = composite.captureStream(FPS);
		const dest = ensureAudioDestination();
		...
		recorder.onerror = (e) => console.error('录制出错：', e);
		recorder.onstop = () => {
			...
			isRecording = false;
			setButtonUI(false);
			console.log('✅ 录制完成，已下载：', FILE_NAME);
		};

		recorder.start();
		isRecording = true;
		setButtonUI(true);
		console.log('🎥 开始录制（高码率）。再次点击 REC 或按 R 停止。');
	}
	...
	return { start, stop, toggle, get isRecording() { return isRecording; } };
})();
...
// 全局调试函数
window.debugFireworkSound = function() {
	console.log('=== 🎆 烟花音效调试信息 ===');
	console.log('🔊 声音启用状态:', store.state.soundEnabled);
	console.log('⏸️  暂停状态:', store.state.paused);
	console.log('📋 菜单打开:', store.state.menuOpen);
	console.log('✅ canPlaySound:', canPlaySoundSelector());
	console.log('🎵 音频上下文状态:', soundManager.ctx.state);
	console.log('🚀 simSpeed:', simSpeed);
	console.log('💾 音频已加载的类型:', Object.keys(soundManager.sources));
	
	// 测试播放
	console.log('🧪 尝试播放测试音效...');
	soundManager.playSound('lift', 0.5);
	setTimeout(() => soundManager.playSound('burst', 0.8), 500);
	setTimeout(() => soundManager.playSound('crackle', 0.6), 1000);
};

console.log('💡 提示: 在控制台输入 debugFireworkSound() 可以查看音效调试信息');
